...
from utils.LoadData import train_srcam_crop_data_loader
...
def get_resnet38_model(args):
    model_name = "models.resnet38_cls1_with_attention"
    print(model_name)
    model = getattr(importlib.import_module(model_name), 'Net')()
    model_local = getattr(importlib.import_module(model_name), 'Net')()

    if len(args.load_checkpoint) == 0:
        # weights_dict = models.resnet38d.convert_mxnet_to_torch('./models/ilsvrc-cls_rna-a1_cls1000_ep-0001.params')
        weights_dict = torch.load("./models/resnet_dict.pth")
...
def train(args):
    ...
    train_loader, val_loader = train_srcam_crop_data_loader(args)
    ...
    while current_epoch < total_epoch:
        ...
        for idx, dat in enumerate(train_loader):
            ...
            # feat_local_label = F.softmax(feat_local_label, dim=1)
            ...
            loss_kd = criterion(feat_aligned, feat_local_label) * 10
            ...
            if global_counter % args.disp_interval == 0:
                print('Epoch: [{}][{}/{}]\t'
                      ...
                      'time:{:.5f}'.format(
                        ...
                        loss_cls_local, batch_time.avg, loss=losses))

        if True or current_epoch == args.epoch-1:
