...
def test_srcam_data_loader(args, ms=True):
    ...
    if ms:
        ...
        img_test = VOCDatasetMSF_srcam(args.test_list, root_dir=args.img_dir, num_classes=args.num_classes,
...
class VOCDataset_srcam(Dataset):
...
# multi scale inference
class VOCDatasetMSF_srcam(Dataset):
...
def train_srcam_sal_data_loader(args, test_path=False, segmentation=False):
    if 'coco' in args.dataset:
        mean_vals = [0.471, 0.448, 0.408]
        std_vals = [0.234, 0.239, 0.242]
    else:
    ...
    img_train = VOCDataset_srcam_sal(args.train_list, root_dir=args.img_dir, sal_dir=args.sal_dir, input_size=input_size, crop_size=crop_size,
                                   num_classes=args.num_classes, patch_num=args.patch_num, transform=tsfm_train, test=False)
    img_test = VOCDataset_srcam(args.test_list, root_dir=args.img_dir, input_size=input_size, crop_size=crop_size,
...
class VOCDataset_srcam_sal(Dataset):
...
class VOCDataset_srcam_sal_crop(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #获得超像素分割的结果
        import skimage.segmentation.slic_superpixels as sp
        #seg = sp.slic(image, n_segments=128, sigma=5).T
        
        #获得边缘检测的结果
        #import cv2
        #cannyEdge = cv2.Canny(image,200,250)
        ...
        if self.transform is not None:
            ...
            seg = sp.slic(image, n_segments=256, sigma=3).T
        ...
        for i in range(len(boxes)):
            ...
            crop_images.append(image[:, box[1]:box[3], box[0]:box[2]].clone()[None])
            crop_sals.append(sal[:, box[1]:box[3], box[0]:box[2]].clone()[None])
            crop_segs.append(seg[ box[1]:box[3], box[0]:box[2]].clone()[None])
            
        crop_images = torch.cat(crop_images, dim=0)
        crop_sals = torch.cat(crop_sals, dim=0)
        crop_segs = torch.cat(crop_segs, dim=0)
        ...
        return image, crop_images, crop_sals, boxes, label, label_local, img_name ,sal  ,seg ,crop_segs #,cannyEdge
...
def train_srcam_sal_crop_data_loader(args, test_path=False, segmentation=False):
    if 'coco' in args.dataset:
        mean_vals = [0.471, 0.448, 0.408]
        std_vals = [0.234, 0.239, 0.242]
    else:
    ...
    img_train = VOCDataset_srcam_sal_crop(args.train_list, root_dir=args.img_dir, sal_dir=args.sal_dir, input_size=input_size, crop_size=crop_size,
...
class VOCDataset_srcam_crop(Dataset):
...
def train_srcam_crop_data_loader(args, test_path=False, segmentation=False):
    if 'coco' in args.dataset:
        mean_vals = [0.471, 0.448, 0.408]
        std_vals = [0.234, 0.239, 0.242]
    else:
    ...
    img_train = VOCDataset_srcam_crop(args.train_list, root_dir=args.img_dir, input_size=input_size, crop_size=crop_size,
                                   num_classes=args.num_classes, transform=tsfm_train, test=False, patch_size=args.patch_size)
    img_test = VOCDataset_srcam(args.test_list, root_dir=args.img_dir, input_size=input_size, crop_size=crop_size,
                              num_classes=args.num_classes, transform=tsfm_test, test=True)
...
class VOCDatasetMSF_srcam_MP(Dataset):
    def __init__(self, datalist_file, root_dir, scales=(0.5, 1.0, 1.5, 2.0), num_classes=20, transform=None,
...
def test_srcam_data_loader_mp(args, ms=True, process_id=0, process_num=3):
    if 'coco' in args.dataset:
        mean_vals = [0.471, 0.448, 0.408]
        std_vals = [0.234, 0.239, 0.242]
    else:
    ...
    if ms:
        ...
        # multi scale inference
        img_test = VOCDatasetMSF_srcam_MP(args.test_list, root_dir=args.img_dir, num_classes=args.num_classes,
